...
- **ローカル Zotero bridge**: `ensure_zotero_bridge.py --ensure` が XPI を Zotero profile に自動インストールまたは更新し、Zotero を再起動してローカル token を取り込みます。`attach_with_bridge.py` は token 保護された `health` / `attach` / `verify` endpoint で PDF を書き戻します。
...
添付ファイルを書き戻す必要がある時、agent が bridge を自動的にインストールします。

```bash
python skills/zotero-translate/scripts/ensure_zotero_bridge.py --ensure
```

`--ensure` は読み込み済み bridge を probe し、bundled manifest の version と比較します。存在しないか古い場合は XPI を作成し、Zotero profile にコピーし、add-on scan cache を消し、Zotero を再起動して token 保護された `health` endpoint を待ちます。この XPI に共有 token は含まれず、Zotero `6.999` から `10.99.99` までの互換性を宣言します。bridge は初回起動時にユーザーごとの `zotero-translate-bridge.json` を Zotero profile に書き込みます。
...
bridge を自動的に確保し、レンダリング済み PDF を添付:
...
| Zotero 添付が失敗する | `ensure_zotero_bridge.py --ensure` を実行し、JSON/stdout/stderr を確認してください。ready になったら、正しい親アイテム ID で `attach_with_bridge.py` を再試行してください。 |
