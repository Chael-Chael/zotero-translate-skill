...
| ローカル Zotero bridge | 添付ファイル書き戻し用の最小ローカル XPI を作成し、profile-side loading を試みます。 |
...
- **ローカル Zotero bridge**: `ensure_zotero_bridge.py` が最小 XPI を作成して profile-side loading を試み、bridge が読み込まれた後に `attach_with_bridge.py` が token 保護された `health` / `attach` / `verify` endpoint で PDF を書き戻します。
