...
1. [`zotero-translate-bridge.xpi`](https://github.com/Chael-Chael/zotero-translate-skill/raw/main/assets/zotero-translate-bridge.xpi)를 다운로드합니다.
2. Zotero에서 `Tools -> Add-ons`를 엽니다.
3. gear icon에서 `Install Add-on From File...`을 선택하고 이 XPI를 고릅니다.
4. Zotero를 재시작합니다.
5. probe를 한 번 실행해 로컬 bridge token을 script가 가져오게 합니다.

```bash
python skills/zotero-translate/scripts/ensure_zotero_bridge.py --probe
```

이 XPI에는 공유 token이 들어 있지 않으며 Zotero `7.0`부터 `9.0.*`까지의 호환성을 선언합니다. bridge는 첫 시작 시 사용자별 `zotero-translate-bridge.json`을 Zotero profile에 씁니다.
...
| Zotero 첨부 실패 | Zotero Add-ons에서 release XPI를 설치하고 Zotero를 재시작한 뒤 `ensure_zotero_bridge.py --probe`를 실행하고, 올바른 상위 항목 ID로 `attach_with_bridge.py`를 다시 시도하세요. |
