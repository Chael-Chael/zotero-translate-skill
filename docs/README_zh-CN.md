...
- **本地 Zotero bridge**：`ensure_zotero_bridge.py --ensure` 会自动把 XPI 安装或更新到 Zotero profile，重启 Zotero，导入本地 token；`attach_with_bridge.py` 通过 token 保护的 `health` / `attach` / `verify` 端点写回 PDF。
...
需要写回附件时，agent 会自动安装 bridge：

```bash
python skills/zotero-translate/scripts/ensure_zotero_bridge.py --ensure
```

`--ensure` 会先 probe 已加载的 bridge，并与 bundled manifest 版本比对；如果缺失或过旧，就构建 XPI、复制到 Zotero profile、清理 add-on 扫描缓存、重启 Zotero，并等待 token 保护的 `health` 端点可用。这个 XPI 不包含共享 token，声明兼容 Zotero `6.999` 到 `10.99.99`；bridge 首次启动会在 Zotero profile 中写入每个用户自己的 `zotero-translate-bridge.json`。
...
| Zotero 附件失败 | 运行 `ensure_zotero_bridge.py --ensure` 并检查 JSON/stdout/stderr；如果已 ready，再用正确的父条目 ID 重试 `attach_with_bridge.py`。 |
