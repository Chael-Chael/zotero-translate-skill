...
| 无 Zotero 插件 | 通过 agent connector 使用 Zotero Desktop；不需要安装单独的 Zotero 翻译插件。 |
...
确定性流程基于 Python，具有可移植性。兼容 agent 只需要能读取 skill 指令、运行本地 Python 脚本，并通过 connector 或本地自动化访问 Zotero Desktop。不需要 Zotero 插件。
...
| 支持 Zotero 的 agent connector | 读取选中条目并附加最终 PDF。 |
...
| Zotero 附件失败 | 确认 Zotero Desktop 已打开，且 agent 的 Zotero connector 可用。 |
