...
| 本機 Zotero bridge | 建立一個最小本機 XPI 用於附件寫回，並嘗試 profile 側載入。 |
...
- **本機 Zotero bridge**：`ensure_zotero_bridge.py` 建立最小 XPI 並嘗試 profile 側載入，`attach_with_bridge.py` 在 bridge 載入後透過 token 保護的 `health` / `attach` / `verify` 端點寫回 PDF。
