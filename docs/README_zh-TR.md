...
| 無 Zotero 外掛 | 透過 agent connector 使用 Zotero Desktop；不需要安裝單獨的 Zotero 翻譯外掛。 |
...
確定性流程基於 Python，具有可移植性。相容 agent 只需要能讀取 skill 指令、執行本機 Python 腳本，並透過 connector 或本機自動化存取 Zotero Desktop。不需要 Zotero 外掛。
...
| 支援 Zotero 的 agent connector | 讀取選中條目並附加最終 PDF。 |
...
你不需要預裝 `pdf2zh`、BabelDOC 或 Zotero 翻譯外掛。skill 會在自己的目錄下準備執行環境。
...
| Zotero 附件失敗 | 確認 Zotero Desktop 已開啟，且 agent 的 Zotero connector 可用。 |
