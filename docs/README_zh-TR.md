...
- **本機 Zotero bridge**：`ensure_zotero_bridge.py --ensure` 會自動把 XPI 安裝或更新到 Zotero profile，重新啟動 Zotero，匯入本機 token；`attach_with_bridge.py` 透過 token 保護的 `health` / `attach` / `verify` 端點寫回 PDF。
...
需要寫回附件時，agent 會自動安裝 bridge：

```bash
python skills/zotero-translate/scripts/ensure_zotero_bridge.py --ensure
```

`--ensure` 會先 probe 已載入的 bridge，並與 bundled manifest 版本比對；如果缺失或過舊，就建立 XPI、複製到 Zotero profile、清理 add-on 掃描快取、重新啟動 Zotero，並等待 token 保護的 `health` 端點可用。這個 XPI 不包含共享 token，宣告相容 Zotero `6.999` 到 `10.99.99`；bridge 首次啟動會在 Zotero profile 中寫入每個使用者自己的 `zotero-translate-bridge.json`。
...
| Zotero 附件失敗 | 執行 `ensure_zotero_bridge.py --ensure` 並檢查 JSON/stdout/stderr；如果已 ready，再用正確的父條目 ID 重試 `attach_with_bridge.py`。 |
