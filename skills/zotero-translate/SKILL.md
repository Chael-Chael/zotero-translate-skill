...
- Attach final PDFs to the original Zotero parent item through the Zotero Translate Bridge. Ensure it with `scripts/ensure_zotero_bridge.py --ensure`; the script probes the loaded bridge, installs or updates the XPI in the Zotero profile when needed, restarts Zotero, clears add-on scan caches, and waits for `health` to report the expected version. Do not ask the user to install the XPI manually unless the automatic script fails and the user explicitly wants a manual fallback.
